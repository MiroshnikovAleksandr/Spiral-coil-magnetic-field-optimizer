...
class GeneticCircular:
    ...
    def __init__(self, params):
        ...
        self.r_max = params['geom']['a_max']
        self.r_min = params['geom']['a_min']
        ...
        self.ind_length = self.discreteness * int(
            (self.r_max - self.r_min) // self.minimal_gap)  # number of digits encoding a chromosome
    ...
    def decode(self, ind: list):
        ...
        for i in range(self.discreteness // 2, len(ones_placement), self.discreteness):
            radii.append(self.r_min + ones_placement[i] * (self.r_max - self.r_min) / self.ind_length)
